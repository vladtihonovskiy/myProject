...
if($_POST!=""&&isset($_POST)&&$_POST) {
    ...
    if ($Category_new != 0 && $product_type_new != 0) {
        $dbus->Setvalue($product_type_new, $Category_new, $_POST["product_name"], $_POST["product_discription"], $_POST["img_url"]);
        header('Location: successpage.php');
    } else {
        if ($Category_new != 0 && $product_type_new == 0) {
            $dbus->Setvalue($_POST['Product_Type'], $Category_new, $_POST["product_name"], $_POST["product_discription"], $_POST["img_url"]);
            header('Location: successpage.php');
        }
        if ($Category_new == 0 && $product_type_new != 0) {
            $dbus->Setvalue($product_type_new, $_POST["Category"], $_POST["product_name"], $_POST["product_discription"], $_POST["img_url"]);
            header('Location: successpage.php');
        }
    }

    if ($Category_new == 0 && $product_type_new == 0) {
        $dbus->Setvalue($_POST['Product_Type'], $_POST["Category"], $_POST["product_name"], $_POST["product_discription"], $_POST["img_url"]);
        header('Location: successpage.php');
    }
}
?>
...
<form id="inputaddform"  method="post">
    <div>
        ...
        <select id="Product_Type" name="Product_Type">
            <option  selected disabled value="Product_Type">Product_Type</option>
            <?php
        ...
        <select id="Category" name="Category" >
            <option selected disabled value="Сategory">Category</option>
            <?php
        ...
        <input type="text" id="product_name" name="product_name" placeholder="Product Name" value="">
        <input type="text" id="product_discription" name="product_discription" placeholder="Product Discription" value="">
        <input type="text" id="Img_url" name="img_url" placeholder="Img Url" value="">

        <button type="submit" id="register" disabled="disabled">Add</button>
    </div>
